...
const ShinyBadgeButton = ({ text, className }) => {
  return (
    <div className="bg-slate-800 no-underline group relative shadow-2xl shadow-zinc-900 rounded-full p-px text-xs font-semibold leading-6 text-white inline-block" >
      ...
      <div className={`relative flex space-x-2 items-center z-10 rounded-full bg-zinc-950 py-0.5 px-4 ring-1 ring-white/10 ${className}`}>
        <span className='text-sm md:text-base font-bold font-space-grotesk'>
          {text}
        </span>
        <svg
          fill="none"
...
ShinyBadgeButton.propTypes = {
  text: PropTypes.string.isRequired,
  className: PropTypes.string,
};

ShinyBadgeButton.defaultProps = {
  className: '',
};

export default ShinyBadgeButton;
